...
function multiply(num1, num2, mycallfunc){
  let prod = num1*num2
  return mycallfunc(prod);
}
...
function first(arr, mycallback){
  return mycallback(arr[0])
}
// UNCOMMENT THE FUNCTION CALL BELOW
// RUN THIS FILE WITH NODE
// CHECK YOUR ANSWER
...
function last(arr, mycallback){
  return mycallback(arr[arr.length-1])
}
...
function contains(array, name, mycaller){
  for (let i =0; i < array.length; i++)
    if (array[i]===name){return mycaller(true)};
  return mycaller(false);
}
// UNCOMMENT THE FUNCTION CALL BELOW
// RUN THIS FILE WITH NODE
// CHECK YOUR ANSWER
...
function each(people, mycall){
  for (let i = 0; i < people.length; i++){
    let item = people[i]
    mycall(item, i) // why does inserting "return" in front of the callback fuction produce a different output?
  }
}
...
const sampleFun = (item, index) => {
  console.log(`The item at index ${index} is ${item}`)
}
each(names, sampleFun);
////////// PROBLEM 7 //////////
...
// CODE HERE 
function getUserByID(array, id, samplecall){
  for (let i = 0; i < array.length; i++){
    let user = array[i];
    if (user.id===id){return samplecall(user)};
  }
}
// UNCOMMENT THE FUNCTION CALL BELOW
// RUN THIS FILE WITH NODE
...
function addingFactory(num){
  console.log("addingFactory being called")
  return funky 
}
